...
def register():
    ...
    form["password"] = generate_password_hash(raw_password)
...
def login():
    ...
    password = request.form.get("password") # Исправлено: получаем пароль из запроса
    
    existing_user = get_user_by_email(email)
    
    # Исправлено: используем check_password_hash
...
def profile():
    ...
    if user_data:
        prin = 7 * "*"
        OTname = user_data['name'][:2] 
        return render_template("profile.html", data=user_data, data_password=prin, OTname=OTname)
...
def delete_profile():
    ...
    # Исправлено: проверка хешированного пароля при удалении
